...
para.tolerance=0.02;%growing region threshold
%para.tolerance=5/255;
para.theta=7*pi/16+pi;%shadow direction toulouse
% para.theta=88*pi/180;%shadow direction lille
para.l=6;
...
veg_like=double(veg_like>0.8);
se_veg=strel('disk',2);
veg_like=imclose(veg_like,se_veg);
figure,subplot(121),imshow(I_rgb),hold on,drawMaskContour(veg_like),subplot(122),imshow(veg_like)
%%
shadow=shadow&(~veg_like);
h_out=figure(5),imshow(I_sub),hold on,drawMaskContour(shadow);
%% generate ROI
para_fuzzy.size=10;
...
ROI=fuzzy_flat(shadow, para_fuzzy);
figure;
imshow(ROI);
...
figure;
imshow(objet);
figure,drawMask(I_rgb,shadow,[1 0 0]),hold on,drawMaskContour(objet_seuil)%,hold on,drawMaskContour(shadow)
...
% end
forgroundMask=(objet_seuil&ROI&(~shadow)&(~veg_like));
forgroundMask(:,1:30)=0;
backgroundMask=(~ROI)|(veg_like);
